...
zika_genes_parsed <- read.table(file = "inst/extdata/zika_genes.tsv", sep="\t", header = T)
...
pred2015 <- read.delim("inst/extdata/Perez-Rueda_2015.tsv", header=T, stringsAsFactors = F, comment.char = "#", na.strings = "")
...
pred2020 <- read.delim("inst/extdata/Flores-Bautista_2020.tsv", header=T, stringsAsFactors = F, comment.char = "#", na.strings = "") %>% 
...
pred2021 <- read.delim("inst/extdata/Kim_2021.tsv", header=T, stringsAsFactors = F, comment.char = "#") 
...
all_tfs <- regulon_tfs %>%
	dplyr::full_join(all_tfs_genes %>% 
									 	...
									 	dplyr::select(RegulonDB_product_name, RegulonDB_tf_id, Ecocyc_id,
									 								...
									 								gene_synonyms, product_synonyms) %>%
									 	tidyr::separate_rows(RegulonDB_tf_id), 
									 by = "RegulonDB_tf_id") %>%
	dplyr::group_by(RegulonDB_tf_id) %>%
